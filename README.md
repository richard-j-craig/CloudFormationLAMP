...
The file NetworkingComponents.yaml will create a new VPC with two subnets, into which the application can be deployed. Note that the code within the metadata sections is automatically created by CloudFormation and is only used to provide a graphic of the components while using the designer. Further details are stated below.
